    ...
    def query_from_db(self, query: str = "", top_k: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            if cache_status:
                ...
                if ids:
                    # Deduplicate IDs to satisfy Chroma get() requirement
                    dedup_ids = list(dict.fromkeys(ids))
                    return self.collection.get(ids=dedup_ids, include=["documents", "metadatas"])
            ...
            if top_ids:
                # Deduplicate before caching to prevent future DuplicateIDError
                dedup_top_ids = list(dict.fromkeys(top_ids))
                success = cache_vectorstore.update_cache(
                    query=query,
                    query_emb=query_embedding,
                    metadata={"ids_json": json.dumps(dedup_top_ids)},
                )
                if success:
                    logging.info(f"Query cached with {len(dedup_top_ids)} ids.")
                else:
